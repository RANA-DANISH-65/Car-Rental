  ...
  useEffect(() => {
    const fetchLikedVehicles = async () => {
      try {
        ...
        console.log("Fetched favorite cars:", validatedData);
      } catch (err) {
        console.error("Failed to fetch liked vehicles:", err.message);
        setFavorites([]); // Prevent null issues
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const renderFavoriteItem = (item) => {
    ...
    return (
      <div
        ...
        className="border rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all bg-[#333] cursor-pointer transform hover:scale-[1.02] duration-300"
        onClick={() => navigateToDetails(item._id)}
      >
        <div className="relative h-52">
          <img
            src={item.carImageUrls[0] || "/placeholder-car.jpg"}
        ...
        <div className="p-4">
          <div className="flex justify-between items-start mb-2">
            <div>
              <h3 className="text-lg font-medium text-green-400 line-clamp-1">
                {item.model.toUpperCase() || "Unnamed Vehicle"}
              </h3>
              <div className="flex items-center mt-1">
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                  {item.manufacturer.toUpperCase() || "Unknown"}
                </span>
              </div>
            </div>
            {item.rating && (
          ...
          <div className="mt-4">
            <button className="w-full bg-green-100 hover:bg-green-300 border border-green-300 rounded-md py-2 px-3 text-sm font-medium text-green-700 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
              View Details
            </button>
          </div>
        </div>
    ...
    <ErrorBoundary>
      <div className="container mx-auto px-4 py-8 text-white max-w-6xl">
        ...
        <p className="text-white mb-6">
          Manage and explore your saved vehicles
        </p>
        ...
        <div className="flex flex-col sm:flex-row justify-between mb-8 bg-green-700 p-4 rounded-lg shadow-sm">
          <div className="mb-4 sm:mb-0">
            ...
            <div className="relative">
              <select
                className="block w-full pl-3 pr-10 py-2 text-base bg-green-500 border-gray-300 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm rounded-md shadow-sm"
          ...
          <div>
            ...
            <div className="relative">
              <select
                className="block w-full pl-3 pr-10 bg-green-500 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md shadow-sm"
                value={sortBy}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 bg-[#333] rounded-lg shadow-sm border border-gray-100">
            ...
            <p className="mt-1 text-gray-300 max-w-sm mx-auto">
              Start exploring our selection of vehicles and add your favorites
              to this list.
            </p>
            <div className="mt-6">
              <button className="px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors" >
                Explore Cars
              </button>
            </div>
          </div>
          ...
          <>
            <p className="text-sm text-gray-300 mb-4">
              ...
              {filteredFavorites.length === 1 ? "vehicle" : "vehicles"}
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {filteredFavorites.length > 6 && (
              <div className="flex justify-center mt-10">
                <button
                  className="flex items-center text-white hover:text-green-800 font-medium"
                  onClick={() =>
