...
const UserTripsPage = () => {
    ...
    const fetchUserTrips = async () => {
      ...
      try {
        ...
        const booking = await fetch(
          "https://car-rental-backend-black.vercel.app/bookings/getBooking"
        );
  ...
  const cancelTrip = async (tripId) => {
    ...
    try {
      ...
      const userId = user ? JSON.parse(user).id : null;
      const response = await fetch(
        "http://192.168.100.17:5000/bookings/cancelBooking",
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl text-green-400 font-bold mb-6">My Trips</h1>

      {/* Tab navigation */}
      <div className="border-b flex justify-center items-center border-gray-200 mb-6">
        <nav className="flex -mb-px">
          {["confirmed", "pending", "completed", "cancelled"].map((tab) => (
            <p
              key={tab}
              onClick={() => handleTabChange(tab)}
              className={`py-4 px-6 cursor-pointer text-center border-b-2 font-medium text-sm ${
                activeTab === tab
                  ? "border-green-500 text-green-700 bg-[#1a1a1a]"
                  : "border-transparent text-gray-300 hover:text-green-700 hover:border-gray-300"
              } transition-colors`}
            >
        ...
        <>
          ...
          {trips.length === 0 ? (
            <div className="text-center py-12 bg-[#333] rounded-lg">
              ...
              <h3 className="mt-2 text-lg font-medium text-green-500">
                No {activeTab} trips
              </h3>
                ...
                >
                  <div className="p-6">
                      ...
                      <div>
                        <div className="flex items-center mb-2">
                          <h2 className="text-xl font-bold text-gray-900 mr-3">
                            {trip.propertyName || "Trip Booking"}
                          </h2>
                          <span
                        ...
                        <p className="text-gray-600 mb-2">
                          ...
                          <span className="font-medium ml-2">Check-out:</span>{" "}
                    ...
                    <div className="mt-4 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                      ...
                      {trip.status === "confirmed" &&
                        new Date(trip.checkInDate) > new Date() && (
                          <button
                            onClick={() =>
                              navigate(`/trips/${trip._id}/modify`)
                            }
                            className="flex-1 bg-white border border-gray-300 rounded-md py-2 px-4 text-sm font-medium text-gray-700 hover:bg-gray-50"
                          >
                            Modify Trip
